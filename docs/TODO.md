...
- [ ] Read IP packets from TUN, hand off to gRPC stream. Sub-items, do one at a time:
  ...
  - [ ] Implement the pump in `internal/tunnel/session/pump_linux.go` (`//go:build linux`): errgroup, two goroutines, sync.Pool of MTU-sized buffers, tag-byte dispatch (0x04 IPv4 / 0x06 IPv6 from the IP version nibble, anything else fails closed), `contracttun.ErrClosed` / `transport.ErrClosed` / `io.EOF` → `nil` return, other errors → wrap-and-return.
  - [ ] Non-Linux stub `internal/tunnel/session/pump_other.go` returning a typed error from `Run` so cross-compile stays green.
