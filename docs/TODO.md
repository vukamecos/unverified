...
- [ ] Read IP packets from TUN, hand off to gRPC stream. Sub-items, do one at a time:
  ...
  - [ ] Define the abstract `Pump` interface (`Run(ctx, device, tunnel) error`) in `internal/contract/session/Pump.go`. Shipped in a future iter (placeholder for the file). Moq-generated mock (per ARCH §13.1).
