...
- [ ] `libbpf-dev`, `clang`, `llvm` (for compiling eBPF programs). Sub-items, do one at a time (split per iter 28 closing summary):
  ...
  - [ ] after 10a — add a scriptable host probe: `clang -target bpf ... -c` round-trip against `/sys/kernel/btf/vmlinux`, producing a known-good CO-RE `.o`, and verify with `llvm-objdump -h` that the `.BTF` section is present. Closes when the probe succeeds end-to-end.
