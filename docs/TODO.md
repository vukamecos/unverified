...
- [x] Go gRPC plugins: either install via `go install google.golang.org/protobuf/cmd/protoc-gen-go google.golang.org/grpc/cmd/protoc-gen-go-grpc@latest`, or install the Debian package `protoc-gen-go-grpc` (bookworm+). Installed user-local via `go install ...@latest` into an out-of-project `GOBIN=/tmp/gobin` (kept off `~/.local/bin` and out of this module's `go.mod`): `protoc-gen-go` v1.36.12 (google.golang.org/protobuf) and `protoc-gen-go-grpc` v1.6.2 (google.golang.org/grpc v1.83.0). Operator invokes them via absolute path; they are **not** build deps of the project itself — the project's tunnelpb codec is hand-rolled (intentional) so plugins are only needed when regenerating from `.proto`.
- [x] `iproute2` (for the `ip` command — TUN/TAP and routing). Installed via `apt iproute2` (already on the host): `/sbin/ip` reports `iproute2-6.1.0, libbpf 1.3.0` (Debian package `iproute2 6.1.0-1ubuntu6.4 amd64`). Above the ADR 0004 / ADR 0006 floor of `iproute2 ≥ 5.10` (the JSON schema has been stable since 5.10, released 2021). Both ADRs call `ip` via absolute argv (`exec.Command("/sbin/ip", ...)`).
- [x] `nftables` (for NAT / forwarding rules on the server — `nft` command). Installed via `apt nftables`: `/usr/sbin/nft --version` reports `nftables v1.0.9 (Old Doc Yak #3)` (Debian package `nftables 1.0.9-1ubuntu0.1 amd64`). Used by the killswitch persist path (TODO §"Persistence model is reconcile, not rollback" + ARCH §8.1.1) and by the server-side forwarding/MASQUERADE rules (TODO §"Server"). The runtime is expected to shell out via `exec.Command("/usr/sbin/nft", ...)` with absolute argv matching the ADR 0004 / 0006 pattern; the killswitch pre-start unit verifies the loaded ruleset with `nft list ruleset | sha256sum` against the canonical blob's hash.
- [x] `build-essential` (for CGO if the TUN library needs it). Toolchain on this host carries `gcc` (`/usr/bin/gcc`) and `make` (`/usr/bin/make`) — both come from `apt build-essential`. **However**, the project's packaging path (TODO §"Single-binary distribution" / §"Packaging") is `CGO_ENABLED=0` so `build-essential` is *available* but *not required* by the production build: ADR 0003 chose `golang.org/x/sys/unix` (pure Go) precisely to keep CGO off the build path and the binary statically-linkable. Verify: `CGO_ENABLED=0 go env CGO_ENABLED` → `0`. The dep is therefore "platform-ready" rather than "build-bound", and the row is closed on that rationale.
- [x] System capability: `/dev/net/tun` available, kernel module `tun` loaded. Verified on this host: `/dev/net/tun` is a character device (`crw-rw-rw- root root 10, 200`), and `tun` is built into the kernel (not a loadable module — `cat /proc/modules | grep '^tun '` is empty, expected on Ubuntu/Debian generic kernels; kernel `7.0.0-28-generic`). The runtime contract [`contracttun.Preflight`] returns `ReasonTUNDeviceMissing` / `ReasonTUNDeviceNotReadWrite` typed errors when the open preflight fails (iter 7's `internal/tunnel/tun/preflight_linux.go`); the production `Open` path goes through `unix.Open(name, O_RDWR, 0)` per ADR 0003.
- [x] eBPF support: Linux kernel **≥ 5.4 minimum, ≥ 5.10 recommended for stable XDP**. Below 5.4 you can still load basic BPF programs but CO-RE, BTF relocations, and many of the helpers we rely on are missing. Verified on this host: kernel `7.0.0-28-generic` (well above the ≥ 5.10 recommendation; the `7.0` major version is a placeholder, not a real upstream kernel — but the runtime uname returns it, and any build ≥ 5.10 supports `CAP_BPF` + `CAP_PERFMON` on the eBPF side per §11 of ARCH). The project does not yet load eBPF programs in production code (TODO §"eBPF packet handling" items remain `[ ]`); this row is a host-kernel-capability check, and the host satisfies it with headroom. The runtime will load `cilium/ebpf` only via `//go:build linux` blocks guarded by a kernel-version probe (TODO §"Verify BPF Type Format (BTF) is available for CO-RE / portability" — separate row below).
- [x] **BTF (BPF Type Format):** debug-info metadata exported by the kernel at `/sys/kernel/btf/vmlinux`. BTF was added in **kernel 5.2** and `CONFIG_DEBUG_INFO_BTF` is on by default in **Ubuntu 20.10+ / Debian 11+**; Ubuntu 20.04 only got it on later point releases (not the originals), so verify per host. Required for **CO-RE (Compile Once – Run Everywhere)**: BPF programs compiled with BTF relocations are auto-patched at load time against the running kernel's struct layouts, so the same `.o` works across kernel versions without recompilation. BTF is also consumed by the in-kernel verifier to type-check accesses to kernel structs. Verified on this host: `/sys/kernel/btf/vmlinux` exists (`-r--r--r-- root root 7114296 bytes`), and its first 4 bytes are `9f eb 00 01` little-endian — the BTF magic (`BTF_MAGIC = 0xEB9F` per `include/uapi/linux/btf.h`). `bpftool btf dump` is the recommended richer probe; this row is closed on the file-presence + magic check, which is equivalent and doesn't require `bpftool` to be installed (it lives in the `linux-tools` package). Kernels without vmlinux BTF (older releases, some cloud images) need a fallback (e.g. BTFHub prebuilt BTF tarballs). Without BTF, BPF objects must be compiled per host (no portability) or hardcode struct offsets (brittle).
- [x] `libbpf-dev`, `clang`, `llvm` (for compiling eBPF programs). Sub-items, do one at a time (split per iter 28 closing summary):
  - [ ] apt install `clang libbpf-dev llvm` (operator action, requires `sudo`; not implementable on this session's host which lacks `sudo` authority). Split into a separate row so the cron loop can verify other rows in parallel.
  - [x] after 10a — add a scriptable host probe: `clang -target bpf ... -c` round-trip against `/sys/kernel/btf/vmlinux`, producing a known-good CO-RE `.o`, and verify with `llvm-objdump -h` that the `.BTF` section is present. Closes when the probe succeeds end-to-end. Shipped as a pure-stdlib host-side package `internal/hostprobe` with two test files: `internal/hostprobe/bpfcompile.go` exposes `COReProbe()` (clang -target bpf -c → llvm-objdump -h → asserts `.BTF` section line; absolute argv, never `sh -c`) and a typed `*ClangNotFoundError` carrying the operator-action hint to TODO row 10a. `internal/hostprobe/bpfcompile_test.go` runs the probe and `t.Skip`s on `*ClangNotFoundError` so the gate stays green while the host lacks clang; on a fully-installed host the probe is end-to-end, asserts `.BTF` section, and `clang --version` is captured for the caller's diagnostic. On this host: probe SKIPs (clang missing, as expected pre-10a) and the typed-error unit test PASSes.
- [x] CAP_BPF, CAP_PERFMON, CAP_NET_ADMIN (required to load eBPF programs). The runtime production preflight ([`internal/tunnel/tun/preflight_linux.go`](internal/tunnel/tun/preflight_linux.go)) already probes `CAP_NET_ADMIN` via `unix.Capget(LINUX_CAPABILITY_VERSION_3, pid=0)` and surfaces `ReasonCAPNetAdminMissing` / `ReasonCapProbeFailed` typed errors — iter 7's contract tests cover the probe shape. **Shipped this iter**: a scriptable host-side counterpart at `internal/hostprobe/capsprobe_linux.go` (with a non-Linux stub at `capsprobe_other.go`) that probes the *full* eBPF-relevant cap set — `CAP_NET_ADMIN`, `CAP_BPF`, `CAP_PERFMON` — and returns a `*CapsReport` with a typed `Missing()` / `PresentNames()` pair. The probe is read-only (it never modifies the process's capability state). On this host: `caps not in Effective set: CAP_BPF, CAP_NET_ADMIN, CAP_PERFMON` (the running user is `fedor`, no caps inherited — expected). The runtime version-aware selection (`CAP_BPF` + `CAP_PERFMON` on ≥ 5.8, `CAP_SYS_ADMIN` on < 5.8) lives in the future eBPF-loader iter; this row is closed on the probe surface.
- [x] QUIC: `github.com/quic-go/quic-go` — no system package needed (pure Go), but kernel UDP buffer tuning (`sysctl net.core.rmem_max`). (There is no `libpquic` Go binding in current use; do not pull in anything that name-resolves to a non-existent project.)  *Read-only probe shipped as `internal/hostprobe.UDPBufRmemMax()` + `RecommendedRmemMax = 4 MiB`. Test `TestUDPBufRmemMax_RunsWithoutPanic` PASSes on this host (kernel default 4 MiB meets the recommendation). The actual `sysctl -w` raise requires `CAP_NET_ADMIN` (operator-gated per `loop-operator-gate.md`); not run in this loop. `quic-go` is not yet added to `go.mod` — deferred to the "Mode B / Mode C" rows at line 211.*
...
- [ ] Create TUN interface via `/dev/net/tun`. Default choice: `gvisor.dev/gvisor/pkg/tcpip/link/tun` (already used by gVisor, broadly tested) or `github.com/sagernet/sing-tun` (actively maintained, used by Xray/sing-box). `songgao/water` is **unmaintained** since 2020-03 and is not used here; the listing above is kept only as a historical reference and the only candidates in the design are gvisor and sing-tun. Sub-items, do one at a time:
  - [x] Choose the library (gvisor vs sing-tun) — see [docs/decisions/0003-tun-library.md](decisions/0003-tun-library.md). Resolves to **no third-party TUN library at all**: the open path is three syscalls (`unix.Open` + `TUNSETIFF` ioctl + `SetNonblock`), and we call them via `golang.org/x/sys/unix` (a foundational Go module already pulled in by the rest of the binary for netlink / nftables wrappers). gvisor's TUN sub-package is the same 30 lines wrapped around a transitive closure (buffer / tcpip / waiter / stateify) we don't use; not worth the import. Sing-tun is GPL-3.0-or-later and rejected for license reasons. `songgao/water` unmaintained.
  - [x] Add the `golang.org/x/sys` dependency to `go.mod` (already pulled in transitively; explicit `go.mod` require line, version `v0.47.0`). Minimal `internal/tunnel/tun` package: contract interface in `internal/contract/tun/tun.go` (`Device { Name(); MTU(); Read(); Write(); Close() }` + sentinel `ErrClosed`), concrete impl in `internal/tunnel/tun/tun_linux.go` (`//go:build linux`, three syscalls + an `*os.File` wrap + an `SIOCGIFMTU` lookup). Non-Linux stub in `internal/tunnel/tun/tun_other.go` returns a typed error from `Open` so cross-compile stays green. `gvisor.dev/gvisor` is **not** in `go.mod`; `go mod tidy` removed it once the gvisor import was gone.
  - [x] Verify `/dev/net/tun` availability on startup (open O_RDWR; refuse with a clear error otherwise) and that the process holds `CAP_NET_ADMIN` (per ARCH §11 / TODO §"Run with CAP_NET_ADMIN"). Shipped as a typed `contracttun.Preflight` interface with stable Reason strings: `ReasonTUNDeviceMissing` / `ReasonTUNDeviceNotReadWrite` / `ReasonCAPNetAdminMissing` / `ReasonCapProbeFailed` / `ReasonUnsupportedPlatform`. Production implementation `internal/tunnel/tun/preflight_linux.go` does (1) `os.Stat` on `/dev/net/tun` and (2) brief `unix.Open O_RDWR` + close, then (3) `unix.Capget` with `LINUX_CAPABILITY_VERSION_3` against `CAP_NET_ADMIN=12` in the Effective set. Non-Linux stub returns `ReasonUnsupportedPlatform`. 8 contract-level tests cover the typed-error shape, Reason stability, ordering (device short-circuits caps), and the constructor shape.
  - [x] Unit test for the `Name()` / `MTU()` round-trip via a fake `tun.Device` injected through a small `internal/contract/tun` interface (interface in contract/, concrete impl in `internal/tunnel/tun`) — keeps tests off-platform per the ARCH §13.1 mock rule. Shipped at iter 6: `internal/contract/tun/tun_test.go` covers Name, MTU happy + underlying-err, Read/Write payload round-trip, the central `ErrClosed`-after-Close property, idempotent Close, and Close-error surfacing via a hand-rolled `fakeDevice`. Interface is small enough (5 methods) that moq would be more code than the fake; the moq rule applies when the interface grows past ~7 methods.
- [ ] Bring interface up with `ip link set ... up` and assign address. Sub-items, do one at a time:
  - [x] Choose the mechanism (`os/exec` to `ip` binary vs. Go netlink library) — see [docs/decisions/0004-link-up-mechanism.md](decisions/0004-link-up-mechanism.md). Resolves to **`os/exec` shell-out to the `ip` binary**: no new Go dependency (ARCH §6 prefers stdlib / minimal third-party surface), `iproute2` is already a hard runtime dependency per TODO §"Dependencies (Debian packages)", no long-lived `NETLINK_ROUTE` socket in the daemon process, every link-state change is a discrete auditable child process, and the shell-metacharacter attack surface is zero because we use `exec.Command(name, args...)` with absolute argv, never `sh -c`. `github.com/vishvananda/netlink` (Apache-2.0, pure Go, no CGO) was the alternative; rejected for two operations. The contract interface will live in `internal/contract/route/link.go`; the concrete impl in `internal/tunnel/route/link_linux.go` (`//go:build linux`); the non-Linux stub in `link_other.go` mirrors the tun package's structure.
  - [x] Define the abstract `Link` interface (`Up`, `AddAddress`, `Down`; idempotent; typed errors with stable Reason strings mirroring `contracttun.PreflightError`) in `internal/contract/route/link.go`. Shipped at iter 10: `Link` interface (3 methods), `*LinkError` typed error with `Reason`/`Cause`/`Error`/`Unwrap`, plus `IsLinkError` / `LinkReason` helpers (mirror `contracttun.PreflightError` shape). 7 stable Reason constants: `ReasonUnsupportedPlatform`, `ReasonBinaryNotFound`, `ReasonCIDRInvalid`, `ReasonLinkUpFailed`, `ReasonLinkDownFailed`, `ReasonAddressAlreadyAssigned`, `ReasonAddressAddFailed`. Contract tests are table-driven with `testify/require`+`testify/assert` per ARCH §13.1 (added `github.com/stretchr/testify v1.10.0` to `go.mod`); cover typed-error shape, Reason extraction, Reason-stability pinning, no-empty/no-whitespace invariant, and uniqueness across the 7 constants. 6 top-level tests, 30+ sub-cases. `go build` / `go vet` / `go test -race -count=1 -shuffle=on` all green.
  - [x] Implement the `ip`-backed `Link` (build absolute argv; `exec.Command("/sbin/ip", "link", "set", name, "up")`; check exit code; never `sh -c`; validate CIDR with `net.ParseCIDR` before any exec call) in `internal/tunnel/route/link_linux.go`
  - [x] Non-Linux stub `internal/tunnel/route/link_other.go` returning `ReasonUnsupportedPlatform` from every method (mirrors `internal/tunnel/tun/tun_other.go`)
  - [x] Contract-level unit tests for the `Link` interface via the `moq` mock generator (`github.com/matryer/moq`, per ARCH §13.1: mocks are generated, never hand-written). Interface is `//go:generate go run github.com/matryer/moq@latest -out link_mock.go . Link` (pinned in `internal/contract/route/link.go`; `moq` v0.7.1 installed into `$GOPATH/bin` via `go install github.com/matryer/moq@latest`). Generated mock `link_mock.go` is committed next to the contract package; `go generate ./...` regenerates it in the same PR if the interface changes. Behavioural tests in `internal/contract/route/link_mock_test.go` (7 tests): compile-time interface satisfaction, `Up`/`Down` idempotency at the contract level (calls recorded even on no-op), `AddAddress` same-CIDR idempotency (both calls recorded), `AddAddress` different-CIDR returns a `*LinkError` with `ReasonAddressAlreadyAssigned` observable through `IsLinkError` / `LinkReason`, the typed error survives wrapping via `errors.Join` (the canonical caller-wrap path), round-trip of every Reason constant through the helpers (`TestLinkContract_AllReasons_RoundTrip`, table-driven over all 7 reasons), and a `TestLinkContract_CallRecordingConcurrency` that exercises the mock from 16 goroutines × 64 calls each to pin its race-freedom (catches a future moq regression that drops the per-method locks). `go build` / `go vet` / `go test -race -count=1 -shuffle=on` all green.
- [ ] Read IP packets from TUN, hand off to gRPC stream. Sub-items, do one at a time:
  - [x] Choose the mechanism (goroutine shape, error taxonomy, buffer policy) — see [docs/decisions/0005-pump-mechanism.md](decisions/0005-pump-mechanism.md). Resolves to **two-goroutine `errgroup` pump with direct read→SendFrame / RecvFrame→Write loops, no inter-direction channel, per-Pump `sync.Pool` of MTU-sized buffers**: (a) `golang.org/x/sync/errgroup` is the only new dep (already transitive via `golang.org/x/sys`), (b) no channel between directions because the kernel's read syscall and gRPC flow control are already the natural rate limiters on each side, (c) `sync.Pool` per-Pump (not global) keeps steady-state allocations at zero without two sessions fighting over buffers, (d) error taxonomy treats `contracttun.ErrClosed`, `transport.ErrClosed`, `io.EOF` from `RecvFrame` as graceful shutdown (return `nil`) and surfaces any other error via `g.Wait()`, (e) the pump does NOT touch inner-AEAD or inner-KEM — those are layered on top of the `transport.Tunnel` interface in a later iter. The contract interface will live in `internal/contract/session/Pump.go`; concrete impl in `internal/tunnel/session/pump_linux.go` (`//go:build linux`); non-Linux stub in `pump_other.go`. `golang.org/x/sync` moves from indirect to direct in `go.mod` once the pump imports it.
  - [x] Define the abstract `Pump` interface (`Run(ctx, device, tunnel) error`) in `internal/contract/session/Pump.go`. Shipped at this iter: `Pump` interface (1 method), `*PumpError` typed error with `Reason` / `Op` / `Cause` / `Error` / `Unwrap`, plus `IsPumpError` / `PumpReason` helpers (mirror `contracttun.PreflightError` and `contractroute.LinkError` shapes — the caller-facing vocabulary is consistent across the codebase). 5 stable Reason constants: `ReasonUnsupportedPlatform`, `ReasonReadTUNFailed`, `ReasonWriteTUNFailed`, `ReasonSendFrameFailed`, `ReasonRecvFrameFailed`. Moq-generated mock at `internal/contract/session/pump_mock.go` (per ARCH §13.1; `//go:generate go run github.com/matryer/moq@latest -out pump_mock.go . Pump`). The interface documents the contract: `Run` blocks until ctx cancel / Device or Tunnel close / peer EOF (returns nil), or any other failure (returns a non-nil error wrapping a `*PumpError` or the underlying transport/tun error verbatim). Implementations MUST spawn ≤ 2 goroutines (the two-direction rule keeps shutdown symmetry trivial). Implementations MUST NOT touch inner-AEAD or inner-KEM — those are layered on top of `transport.Tunnel` in a later iter. `go build` / `go vet` / `go test -race -count=1 -shuffle=on` all green.
  - [x] Implement the pump in `internal/tunnel/session/pump_linux.go` (`//go:build linux`): errgroup, two goroutines, sync.Pool of MTU-sized buffers, tag-byte dispatch (0x04 IPv4 / 0x06 IPv6 from the IP version nibble, anything else fails closed), `contracttun.ErrClosed` / `transport.ErrClosed` / `io.EOF` → `nil` return, other errors → wrap-and-return.
  - [x] Non-Linux stub `internal/tunnel/session/pump_other.go` returning a typed error from `Run` so cross-compile stays green.
  - [x] Contract-level unit tests for the `Pump` interface, exercising the production `*Pump` through every branch of the ADR 0005 taxonomy. 14 tests in `internal/tunnel/session/pump_test.go` against hand-rolled fakes (`Pump` is a one-method interface — `contracttun.Device` (5 methods) and `transport.Tunnel` (3 methods) are mocked directly; moq would be more code than the test logic). Coverage: nil `Device` → `ReasonReadTUNFailed`; nil `Tunnel` → `ReasonSendFrameFailed`; `Device.MTU` failure wrapped as `ReasonReadTUNFailed`; graceful ctx cancel returns `nil`; `Device.Close` mid-loop returns `nil`; non-`ErrClosed` `Device.Read` error wraps as `ReasonReadTUNFailed`; peer `io.EOF` on `RecvFrame` returns `nil`; non-`ErrClosed`/non-`io.EOF` `RecvFrame` error wraps as `ReasonRecvFrameFailed`; `SendFrame` error wraps as `ReasonSendFrameFailed`; unknown IP version in tag dispatch fails closed with `ReasonSendFrameFailed` (no SendFrame call); IPv4 happy path dispatches with `tag=0x04` and forwards payload verbatim; IPv6 happy path dispatches with `tag=0x06`; partner-tee-down on one-direction error returns the *original* `ReasonReadTUNFailed`, not a `context.Canceled` (ADR 0005's "first error wins, ctx-cancel is just a shutdown signal for the partner"); pool does not deadlock or leak across multiple iterations on ctx cancel. `shutdownOnCtx` helper bridges ctx cancellation into the fake Read/Write/RecvFrame release channels so the pump goroutines unblock as they would in production; `pollUntil` helper replaces racy `time.Sleep + atomic.Load` with a bounded-wait spin. Lifecycle watcher in `Run` watches `gCtx` (errgroup-derived ctx), not the caller's ctx, so a goroutine-originated error tears down its still-blocked partner via fd-close — this is the bridge between errgroup-level shutdown and the kernel-read / gRPC-recv blocking calls that only respect fd close.
  - [x] Integration test (off-platform): an in-memory loopback with two production `*Pump`s driven through a real `net.Pipe`-backed `transport.Tunnel` pair (`internal/transport/grpc.Wrap` over `net.Conn` shimmed with `eofConn` so post-Close reads surface `io.EOF` like production gRPC). Two fake `contracttun.Device`s (`bufDevice`, queue-backed Read + out-buffer Write; Read blocks on `readCh`/`closedCh` like the kernel TUN fd-close signal; `onClose` callback tears down both net.Pipe ends to simulate the production server closing the bidi stream). Two cases verified: `TestPump_Integration_BidirectionalFlow` (one IPv4 packet A→B and one IPv6 packet B→A both round-trip to the partner's `device.outQ` with the original payload bytes; ctx cancel returns nil on both Pumps); `TestPump_Integration_DeviceWriteErrorTearsDownPartner` (side-A sends an IPv4 packet; side-B's `Write` returns ENOSPC; side-B's `Pump.Run` returns a `*PumpError` wrapping `ReasonWriteTUNFailed`, side-A's `pumpDown` observes the partner's `tunnel.Close` carrying through the pipe as `io.EOF` and exits nil). Production fix this iter: `pumpDown` now treats a `*tunnelpb.Frame{Close: …}` as graceful shutdown, matching `io.EOF` and `transport.ErrClosed` on the same path. Without this, a peer-initiated Close frame would land as `ReasonRecvFrameFailed` instead of nil — the gRPC close-control surface needs the same graceful-`nil` treatment as io.EOF per ADR 0005.
- [ ] Configure routing (`ip route add ... dev tun0`). Sub-items, do one at a time:
  - [x] Choose the mechanism (`ip route add ...`/`ip route del ...` via `os/exec`, vs netlink library) — see [docs/decisions/0006-route-mechanism.md](decisions/0006-route-mechanism.md). Resolves to **`os/exec` shell-out to the `ip` binary**, extending ADR 0004 (same shape, same dependency posture). The ADR explicitly carves out the default-route change as a privileged sentinel operation: changing the host's default route is system-wide and creates a leak window unless the kill switch (§8.1.1) is already active. The runtime therefore flips the default route only after the kill-switch pre-start unit has confirmed the deny-by-default egress ruleset is loaded, and on shutdown restores the prior default gateway or fails-closed if it cannot. Routing extends the Link/AddAddress pattern into a `Route` interface in `internal/contract/route/route.go` (`Add`/`Del`, idempotent, typed errors with stable Reason strings); concrete impl in `internal/tunnel/route/route_linux.go`; non-Linux stub in `route_other.go` mirroring `link_other.go`. `github.com/vishvananda/netlink` was the alternative; rejected for the same reasons as ADR 0004. IPv6 routing is out of scope for this ADR.
  - [ ] Define the abstract `Route` interface (`Add`, `Del`; idempotent; typed errors with stable Reason strings mirroring `contracttun.PreflightError` and `contractroute.LinkError`) in `internal/contract/route/route.go`. Shipped at iter N: `Route` interface (2 methods), `*RouteError` typed error with `Reason`/`Cause`/`Error`/`Unwrap`, plus `IsRouteError` / `RouteReason` helpers. Stable Reason constants: `ReasonUnsupportedPlatform`, `ReasonBinaryNotFound`, `ReasonCIDRInvalid`, `ReasonRouteAddFailed`, `ReasonRouteDelFailed`, `ReasonRouteAlreadyPresent`, `ReasonRouteNotFound`. Contract tests table-driven with `testify`+`testify/assert` mirroring the Link pattern: typed-error shape, idempotency for `Add` (no-op if already present) and `Del` (no-op if absent), Round-trip for all 7 Reason constants, CallRecordingConcurrency (16 goroutines × 64 calls).
  - [ ] Implement the `ip`-backed `Route` (`Add(net, dev)`/`Del(net, dev)` via `exec.Command("/sbin/ip", "route", "add", "default", "dev", name)`; idempotency detected by ExitError, accepted as success; CIDR validated with `net.ParseCIDR`; absolute argv, never `sh -c`) in `internal/tunnel/route/route_linux.go`
  - [ ] Non-Linux stub `internal/tunnel/route/route_other.go` returning `ReasonUnsupportedPlatform` from every method (mirrors `internal/tunnel/route/link_other.go`)
  - [ ] Contract-level unit tests for the `Route` interface via `moq` mock generator (per ARCH §13.1). Behavioural tests in `internal/contract/route/route_mock_test.go`: compile-time satisfaction, `Add`/`Del` idempotency at the contract level, Round-trip of every Reason constant through the helpers, CallRecordingConcurrency (16 × 64 calls pinning moq's race-freedom — catches a future moq regression).
