...
- [ ] Read IP packets from TUN, hand off to gRPC stream. Sub-items, do one at a time:
  ...
  - [ ] Integration test (off-platform): an in-memory loopback with two `Pumps`, two fake `Tunnel`s (a `pipe`-backed `Transport`), and two fake `Devices` (`bytes.Buffer`-backed). Verifies bidirectional flow, graceful shutdown on either side, and that a write error in one direction tears down the other.
