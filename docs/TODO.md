...
- [ ] Bring interface up with `ip link set ... up` and assign address. Sub-items, do one at a time:
  ...
  - [ ] Define the abstract `Link` interface (`Up`, `AddAddress`, `Down`; idempotent; typed errors with stable Reason strings mirroring `contracttun.PreflightError`) in `internal/contract/route/link.go`
