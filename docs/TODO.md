...
- [ ] Bring interface up with `ip link set ... up` and assign address. Sub-items, do one at a time:
  ...
  - [ ] Implement the `ip`-backed `Link` (build absolute argv; `exec.Command("/sbin/ip", "link", "set", name, "up")`; check exit code; never `sh -c`; validate CIDR with `net.ParseCIDR` before any exec call) in `internal/tunnel/route/link_linux.go`
  - [ ] Non-Linux stub `internal/tunnel/route/link_other.go` returning `ReasonUnsupportedPlatform` from every method (mirrors `internal/tunnel/tun/tun_other.go`)
