...
- [ ] Read IP packets from TUN, hand off to gRPC stream. Sub-items, do one at a time:
  ...
  - [ ] Contract-level unit tests for the `Pump` interface via the `moq` mock generator (`github.com/matryer/moq`). Interface is `//go:generate go run github.com/matryer/moq@latest -out pump_mock.go . Pump`. Behavioural tests cover: pump returns `nil` on peer EOF (Tunnel mock returns `io.EOF`); pump returns `nil` on `contracttun.ErrClosed` from `Device.Read`; pump wraps & returns errors from `Device.Read`; pump wraps & returns errors from `Tunnel.SendFrame`; pump cancels the partner goroutine when one direction errors; pump cleans up the buffer pool on ctx cancel.
