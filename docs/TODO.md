...
- [ ] Bring interface up with `ip link set ... up` and assign address. Sub-items, do one at a time:
  ...
  - [ ] Contract-level unit tests for the `Link` interface via the `moq` mock generator (`github.com/matryer/moq`, per ARCH §13.1: mocks are generated, never hand-written). Interface is `//go:generate moq -out link_mock.go . Link`. Generated mock is committed next to the contract package; `go generate ./...` regenerates it in the same PR if the interface changes.
